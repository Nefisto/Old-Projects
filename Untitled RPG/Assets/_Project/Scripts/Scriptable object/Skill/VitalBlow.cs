...
[CreateAssetMenu(fileName = "Vital blow",
    menuName = EditorConstants.MenuAssets.ACTIVE_SKILLS + "Vital blow", order = 0)]
public class VitalBlow : LevelChargeSkill
{
    [TitleGroup("Settings")]
    ...
    protected override IEnumerator BeforeCalculateActionValues (BattleActionContext context)
    {
        foreach (var actionInfo in context)
        {
            ...
            if (GetChargeLevel > 1)
                missingHealthPercentage += 1f - actionInfo.caster.HealthResource.CurrentPercentage;
