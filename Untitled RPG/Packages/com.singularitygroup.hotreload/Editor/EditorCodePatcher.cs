...
#pragma warning restore CS0612 // Type or member is obsolete
        ...
        internal static bool Starting
            => (StartedServerRecently() || ServerHealthCheck.I.IsServerHealthy)
               && !Started
               && starting
               && patchStatus != PatchStatus.CompileError;

        internal static bool Stopping => stopping && Running;

        internal static bool Compiling
            => DateTime.UtcNow - startWaitingForCompile < TimeSpan.FromSeconds(5)
               || patchStatus == PatchStatus.Compiling
               || HotReloadRunTab.recompiling;
        ...
        [CanBeNull]
        internal static LoginStatusResponse Status { get; private set; }
        ...
        static async Task FlushErrors()
        {
            ...
            if (response == null || !Application.isPlaying)
            {
                return;
            }
        ...
        static void OnIntervalMainThread()
        {
            ...
            {
                TryPrepareBuildInfo();
                if (!requestingCompile && Application.isPlaying)
                {
                    RequestHelper.PollMethodPatches(HotReloadState.LastPatchId, HandleResponseReceived);
                }
                ...
                RequestHelper.PollPatchStatus(resp =>
                {
                    ...
                    if (patchStatus == PatchStatus.Patching)
                    {
                        ...
                        {
                            EditorWindowHelper.ShowNotification(EditorWindowHelper.NotificationStatus.Patching,
                                maxDuration: 10);
                        }
                    }
        ...
        static void HandleResponseReceived (MethodPatchResponse response)
        {
            ...
            else if (_applyingFailed)
            {
                ...
                if (patchResult?.patchFailures.Count > 0)
                {
                    foreach (var failure in patchResult.patchFailures)
                    {
                        ...
                        HotReloadTimelineHelper.CreatePatchFailureEventEntry(error, methodName: GetMethodName(method),
                            methodSimpleName: method.simpleName, entryType: EntryType.Child);
                    }
                }
            ...
            else if (_appliedPartially)
            {
                ...
                {
                    HotReloadTimelineHelper.CreatePartiallyAppliedEventEntry(responsePartiallySupportedChange,
                        entryType: EntryType.Child, detailed: false);
                }
        ...
        internal static void HandleStatus (LoginStatusResponse resp)
        {
            ...
            bool consumptionsChanged = Status?.freeSessionRunning != resp.freeSessionRunning
