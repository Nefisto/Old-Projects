    ...
    public class MMF_Player : MMFeedbacks
    {
        ...
        public virtual bool IsAllowedToPlay (Vector3 position)
        {
            ...
            if (OnlyPlayIfWithinRange)
            {
                if (RangeCenter == null)
                    return false;
                var distanceToCenter = Vector3.Distance(position, RangeCenter.position);
        ...
        protected override IEnumerator PausedFeedbacksCo (Vector3 position, float feedbacksIntensity)
        {
            ...
            while (i >= 0 && i < count)
            {
                ...
                if ((FeedbacksList[i].Active && FeedbacksList[i].ScriptDrivenPause) || InScriptDrivenPause)
                {
                    ...
                    var scriptDrivenPauseStartedAt = GetTime();
        ...
        ///     a flickering renderer. It's usually called automatically before playing them.
        /// </summary>
        ...
        /// <summary>
        ///     Pauses execution of a sequence, which can then be resumed by calling ResumeFeedbacks()
        ///     Note that this doesn't stop feedbacks, by design, but in most cases you'll probably want to call StopFeedbacks()
        ///     first
        /// </summary>
        ...
        public virtual void ForceInitialValues()
        {
            ...
            for (var i = count - 1; i >= 0; i--)
                if (FeedbacksList[i] != null && FeedbacksList[i].Active)
                    FeedbacksList[i].ForceInitialValue(transform.position, FeedbacksIntensity);
        }

        /// <summary>
        ///     Skips to the end of a sequence of feedbacks. Note that depending on your setup, this can take up to 3 frames to
        ///     complete, don't disable your player instantly, or it won't complete the skipping
        /// </summary>
        ...
        private class MMF_FeedbackListCopy
        {
            ...
            {
                var listCopy = new MMF_FeedbackListCopy();
                listCopy.FeedbackList = source.FeedbacksList;
                var json = JsonUtility.ToJson(listCopy);
        ...
        {
            var tempList = new List<MMF_Feedback>();
            var tempList2 = new List<MMF_Feedback>();
        ...
        public virtual float ComputeRangeIntensityMultiplier (Vector3 position)
        {
            ...
            var normalizedDistance = MMFeedbacksHelpers.Remap(distanceToCenter, 0f, RangeDistance, 0f, 1f);
            var curveValue = RangeFalloff.Evaluate(normalizedDistance);
            var newIntensity = MMFeedbacksHelpers.Remap(curveValue, 0f, 1f, RemapRangeFalloff.x, RemapRangeFalloff.y);
            return newIntensity;
        }
        ...
        public override float ApplyTimeMultiplier (float duration)
            => duration * Mathf.Clamp(DurationMultiplier, _smallValue, float.MaxValue) * _randomDurationMultiplier;
        ...
        public virtual T GetFeedbackOfType<T> (AccessMethods method, int referenceIndex) where T : MMF_Feedback
        {
            ...
            switch (method)
            {
                case AccessMethods.First:
                    for (var i = 0; i < FeedbacksList.Count; i++)
                        if (Check(i))
                ...
                case AccessMethods.Closest:
                    var closestIndexBack = referenceIndex;
                    var closestIndexForward = referenceIndex;
                    for (var i = referenceIndex; i >= 0; i--)
                        if (Check(i))
                        {
                            closestIndexBack = i;
                    ...
                    for (var i = referenceIndex; i < FeedbacksList.Count; i++)
                        if (Check(i))
                        {
                    ...
                    if (closestIndexBack != referenceIndex || closestIndexForward != referenceIndex)
                    {
                        ...
                        {
                            var distanceBack = Mathf.Abs(referenceIndex - closestIndexBack);
                            var distanceForward = Mathf.Abs(referenceIndex - closestIndexForward);
            ...
            bool Check (int i) => FeedbacksList[i].GetType() == _t;
        }
        ...
        public virtual void ComputeCachedTotalDuration()
        {
            ...
            {
                var lastLooperStart = 0;
                var lastLoopFoundAt = 0;
                var lastPauseFoundAt = 0;
                var loopsLeft = 0;
                var iterations = 0;
                var maxIterationsSafety = 1000;
                var currentPauseDelay = 0f;
                var i = Direction == Directions.TopToBottom ? 0 : Feedbacks.Count - 1;
                var intermediateTotal = 0f;
                        ...
                        && FeedbacksList[i].ShouldPlayInThisSequenceDirection)
                    {
                        ...
                        if (FeedbacksList[i].Pause != null)
                        {
                            ...
                            if (FeedbacksList[i].LooperPause && (FeedbacksList[i] as MMF_Looper).NumberOfLoops > 0)
                            {
                                ...
                                if ((FeedbacksList[i] as MMF_Looper).LoopAtLastLoopStart)
                                {
                                    i = lastLooperStart;
                                    total += intermediateTotal;
                                    intermediateTotal = 0f;
                        ...
                        {
                            var feedbackDuration = FeedbacksList[i].TotalDuration + currentPauseDelay;
