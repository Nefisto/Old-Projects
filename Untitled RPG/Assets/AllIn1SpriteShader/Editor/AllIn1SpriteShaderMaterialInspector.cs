...
public class AllIn1SpriteShaderMaterialInspector : ShaderGUI
{
    ...
    private readonly uint[] materialDrawers = { 1, 2, 4 };
    ...
    public override void OnGUI (MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        var iniDrawers = (uint)FindProperty("_EditorDrawers", matProperties).floatValue;
        for (var i = 0; i < materialDrawers.Length; i++)
            currEnabledDrawers[i] = (materialDrawers[i] & iniDrawers) > 0;
        ...
        if (currEnabledDrawers[colorFxShapeDrawer])
        {
            ...
            GenericEffect("Color Swap", "COLORSWAP_ON", 36, 42, true, "You will need a mask texture (see Documentation)", new[] { 154 });
            ...
            GenericEffect("Ghost", "GHOST_ON", 64, 65, true, "This effect will not affect the outline", new[] { 157 });
            GenericEffect("Hologram", "HOLOGRAM_ON", 73, 77, true, null, new[] { 140, 158 });
            GenericEffect("Chromatic Aberration", "CHROMABERR_ON", 78, 79);
            GenericEffect("Glitch", "GLITCH_ON", 80, 80, true, null, new[] { 139 });
    ...
    {
        uint currDrawers = 0;
        for (var i = 0; i < currEnabledDrawers.Length; i++)
            if (currEnabledDrawers[i])
                currDrawers |= materialDrawers[i];

        if (iniDrawers != currDrawers)
            FindProperty("_EditorDrawers", matProperties).floatValue = currDrawers;
    }

    private void Blending()
    {
        var srcM = FindProperty("_MySrcMode", matProperties);
        var dstM = FindProperty("_MyDstMode", matProperties);
        ...
        var ini = oldKeyWords.Contains("PREMULTIPLYALPHA_ON");
        var toggle = EditorGUILayout.Toggle("Premultiply Alpha?", ini);
        if (ini != toggle)
            Save();
        if (toggle)
    ...
    {
        var toggle = oldKeyWords.Contains(keyword);
        var ini = toggle;

        var effectNameLabel = new GUIContent();
        ...
        if (toggle)
        {
            ...
            var billboardY = matProperties[129];
    ...
    private void ZWrite()
    {
        var zWrite = FindProperty("_ZWrite", matProperties);
        var toggle = zWrite.floatValue > 0.9f ? true : false;
        EditorGUILayout.BeginHorizontal();
        {
            var tempValue = zWrite.floatValue;
            toggle = GUILayout.Toggle(toggle, new GUIContent("Enable Z Write"));
            if (toggle)
                zWrite.floatValue = 1.0f;
            else
                zWrite.floatValue = 0.0f;
            if (tempValue != zWrite.floatValue && !Application.isPlaying)
                Save();
        }
    ...
    private void ZTest()
    {
        var zTestM = FindProperty("_ZTestMode", matProperties);
        var tempValue = zTestM.floatValue;
    ...
    {
        var cullO = FindProperty("_CullingOption", matProperties);
        ;
        var tempValue = cullO.floatValue;
    ...
    private void Outline (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                ...
                DrawLine(Color.grey, 1, 3);
                var outlinePixelPerf = DrawEffectSubKeywordToggle("Outline is Pixel Perfect?", "OUTBASEPIXELPERF_ON");
                if (outlinePixelPerf)
                    DrawProperty(18);
                else
                    DrawProperty(17);
                ...
                DrawLine(Color.grey, 1, 3);
                var outlineTexture = DrawEffectSubKeywordToggle("Outline uses texture?", "OUTTEX_ON");
                if (outlineTexture)
                {
                    DrawProperty(19);
                    DrawProperty(20);
                ...
                DrawLine(Color.grey, 1, 3);
                var outlineDistort = DrawEffectSubKeywordToggle("Outline uses distortion?", "OUTDIST_ON");
                if (outlineDistort)
                {
                    DrawProperty(22);
                    DrawProperty(23);
    ...
    private void GenericEffect (string inspector, string keyword, int first, int last, bool effectCounter = true, string preMessage = null, int[] extraProperties = null, bool boldToggleLetters = true)
    {
        ...
        if (toggle)
        {
            ...
            if (first > 0)
            {
                ...
                {
                    if (preMessage != null)
                        GUILayout.Label(preMessage, smallLabelStyle);
                    for (var i = first; i <= last; i++)
                        DrawProperty(i);
                    if (extraProperties != null)
                        foreach (var i in extraProperties)
                            DrawProperty(i);
                }
                EditorGUILayout.EndVertical();
            }
    ...
    private void Glow (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                var useGlowTex = DrawEffectSubKeywordToggle("Use Glow Texture?", "GLOWTEX_ON");
                if (useGlowTex)
                    DrawProperty(6);
        ...
        {
            targetMat.DisableKeyword("GLOW_ON");
        }
    ...
    private void ColorRamp (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                ...
                var colorRampOut = matProperties[53];
    ...
    private void GreyScale (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                ...
                var greyScaleOut = matProperties[55];
    ...
    private void Posterize (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                ...
                var posterizeOut = matProperties[59];
    ...
    private void Grass (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                ...
                var grassManual = matProperties[92];
                if (grassManual.floatValue == 1)
                    targetMat.EnableKeyword("MANUALWIND_ON");
                else
                    targetMat.DisableKeyword("MANUALWIND_ON");
            }
            EditorGUILayout.EndVertical();
        }
        else
        {
            targetMat.DisableKeyword("WIND_ON");
        }
        ...
        if (toggle)
        {
            ...
            if (first > 0)
            {
                EditorGUILayout.BeginVertical(propertiesStyle);
                {
                    ...
                    var onlyInOutline = matProperties[72];
                    if (onlyInOutline.floatValue == 1)
                        targetMat.EnableKeyword("ONLYINNEROUTLINE_ON");
                    else
                        targetMat.DisableKeyword("ONLYINNEROUTLINE_ON");
                }
                EditorGUILayout.EndVertical();
            }
        }
    ...
    private void Gradient (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                ...
                var gradIsRadial = matProperties[143];
                ...
                else
                {
                    ...
                    var simpleGradient = oldKeyWords.Contains("GRADIENT2COL_ON");
                    var simpleGradToggle = EditorGUILayout.Toggle("2 Color Gradient?", simpleGradient);
    ...
    private void Overlay (string inspector, string keyword)
    {
        ...
        if (toggle)
        {
            ...
            EditorGUILayout.BeginVertical(propertiesStyle);
            {
                var multModeOn = oldKeyWords.Contains("OVERLAYMULT_ON");
                var isMultMode = multModeOn;
    ...
    {
        var targetProperty = matProperties[index];

        EditorGUILayout.BeginHorizontal();
        {
            var propertyLabel = new GUIContent();
            ...
            if (!noReset)
            {
                var resetButtonLabel = new GUIContent();
    ...
    private bool DrawEffectSubKeywordToggle (string inspector, string keyword, bool setCustomConfigAfter = false)
    {
        ...
        var ini = oldKeyWords.Contains(keyword);
        var toggle = ini;
