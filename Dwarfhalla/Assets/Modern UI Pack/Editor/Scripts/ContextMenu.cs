...
namespace Michsky.MUIP
{
    public class ContextMenu : Editor
    {
        ...
        {
            EditorUtility.DisplayDialog("Modern UI Pack",
                "Cannot create the object due to missing manager file. "
        ...
        static void CreateObject (string resourcePath)
        {
            try
            {
                ...
                GameObject clone =
                    Instantiate(
                        AssetDatabase.LoadAssetAtPath(objectPath + resourcePath + ".prefab", typeof(GameObject)),
        ...
        static void CreateCanvas()
        {
            try
            {
                ...
                GameObject clone =
                    Instantiate(
                        AssetDatabase.LoadAssetAtPath(objectPath + "Other/Canvas" + ".prefab", typeof(GameObject)),
                        Vector3.zero, Quaternion.identity) as GameObject;
                clone.name = clone.name.Replace("(Clone)", "").Trim();
                Selection.activeObject = clone;
                MakeSceneDirty(clone, clone.name);
        ...
        static void ShowManager()
        {
            ...
            if (Selection.activeObject == null)
                Debug.Log(
                    "<b>[Modern UI Pack]</b>Can't find a file named 'MUIP Manager'. Make sure you have 'MUIP Manager' file in Resources folder.");
        }
